...
const help: Command = {
    ...
    execute(message, args, games, commands) {
        ...
        if (command.aliases) helperEmbed.addField("Aliases", command.aliases.join(', '), true);

        helperEmbed.addField("Cooldown", command.cooldown || 3, true);

        if (command.description) helperEmbed.addField("Description", command.description, false);

        if (command.usage) {
            helperEmbed.addField("Usage",
                command.usage.map(mode => `\`${prefix}${command.name} ${mode}\``).join('\n'), false);
        }
        if (command.argExplanation) helperEmbed.addField("Usage Explained", command.argExplanation), false;
